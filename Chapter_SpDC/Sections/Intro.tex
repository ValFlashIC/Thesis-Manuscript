...
\lettrine{I}{n} the previous chapters, we analyzed the causes and effects of irregular memory accesses in sparse computations and identified the \acrfull{op} algorithm of \acrfull{spmv} ($A \times b = c$) as an important example.
The \acrshort{op} algorithm generates irregular updates on the output vector $c$, as each non-zero element of the input matrix $A$ produces updates to distinct, and often distant, location in the output vector $c$.
...
In contrast, \acrshort{op} \acrshort{spmv} exhibits poor locality, leading to low cache utilization, frequent evictions, and thus requiring more memory bandwidth.
