...
When performing an \acrshort{op} \acrshort{spmv} ($A \times b = c$) with this format (Algorithm~\ref{alg:spdc:spmv_with_red}), we see that a \emph{RED} is needed to accumulate the output vector $c$ (line~6).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{Chapter_SpDC/Figures/PDF/HardwareSpecs_SpMVonMatrix.pdf}
    \caption{Traversal of Non-Zero Elements of Input Matrix for \acrshort{op} \acrshort{spmv} (left) and Impact on Cache (right) \note{(TODO use remaining place to clarify figure / add the pink line which is brought back)}}
...
This is the main idea of SpDCache (Paper~\ref{publ:SpDC}), which is a \gls{redC} with dedicated memory regions for the in-band and out-of-band of a matrix.
...
\begin{highlight}{Conclusion}{Red}
    ...
    \bigskip \\
    We propose \textbf{SpDCache}, a \gls{redC} with dedicated memory regions, \textbf{leveraging matrix structure} with denser and sparser regions, such as \textbf{\gls{banded} matrices}.
\end{highlight}