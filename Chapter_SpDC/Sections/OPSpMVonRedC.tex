...
\begin{algorithm}[htb]
    ...
    %
    \For(\tcp*[h]{Loop over the columns of A}){$n \in \ldb 0, N-1 \rdb$}{
        \For{$pos \in \ldb ptr_n, ptr_{n+1}-1 \rdb$}{
            $key \gets idx_{pos}$\tcp*[l]{Row index}
            $val \gets val_{pos} \times b_n$\tcp*[l]{val = A\textsubscript{key,n} {\tm} b\textsubscript{n}}
            $\mathit{RED}(key, val)$\tcp*[l]{c\textsubscript{key} += val}
        }
    }
\end{algorithm}

In memory, $A$ is stored as three dense tables (\acrshort{csc}), but conceptually, as we perform the \acrshort{op} multiplication, the matrix is traversed from top to bottom, then left to right.
...
The globally low local and temporal localities prevent caches, and even \glspl{redC}, to take advantage of any reuse opportunity.
However, sparse matrices have various types of structure which may have reuse potential in some parts of the matrix.
In the example in Figure~\ref{fig:spdc:spmv_on_matrix}, the first rows (with $\mathit{key}_{10}$ to $\mathit{key}_{13}$) are denser than the rest of the matrix.
...
On the contrary, the sparser rows of the matrix would not be kept by the ``smarter'' cache, thus avoiding thrashing data from denser regions.

The matrix structure is key to designing an efficient \gls{redC} for \acrshort{op} \acrshort{spmv}.
Going forward, we focus on the case of \gls{banded} matrices, a widely used type of sparse matrix, which have a denser region around the diagonal.
For a \gls{redC} to work efficiently with such matrices, the dense \gls{band} should be prioritized, and the isolated elements out of the \gls{band} should be processed separately to avoid these sparse elements disturbing the storage of the dense data.
This is the main idea of \acrlong{spdc} (Paper~\ref{publ:SpDC}), which is a \gls{redC} with dedicated memory regions for the in-\gls{band} and out-of-\gls{band} of a matrix.
We describe its behavior in the following sections.

\begin{highlight}{Conclusion}{Red}
    When computing an \textbf{\acrshort{op} \acrshort{spmv} kernel} on a very \textbf{large matrix}, a \textbf{conventional \gls{redC} does not provide enormous benefit} because the cache is not large enough to reuse data from one column iteration to the next.
    \bigskip \\
    We propose \textbf{\acrlong{spdc}}, a \gls{redC} with dedicated memory regions, \textbf{leveraging matrix structure} with dense and sparse regions which correspond to the structure of \textbf{\gls{banded} matrices}.
\end{highlight}