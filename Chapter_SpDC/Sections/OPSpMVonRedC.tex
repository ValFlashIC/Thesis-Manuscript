...
When performing an \acrshort{op} \acrshort{spmv} ($A \times b = c$) with this format (Algorithm~\ref{alg:spdc:spmv_with_red}), we see that a \gls{red} is needed to accumulate the output vector $c$ (line~6).
...
Each of these results in a \gls{red} on $c$.
