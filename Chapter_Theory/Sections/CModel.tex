...
% cache parameters to define earlier: S, W, l
% gc with prefetcher earlier too
...
The matrix density distribution thus takes on two different values, either $d_\mathit{IB}$ in the band, or $d_\mathit{OB}$ outside of the band, as shown in Figure~\ref{fig:theory:model:banded_mat_def}.
...
In the fast simulation phase, the analytical model~\cite{FRIEDENTHAL2012_AnalyticalModel} iterates over the vertical-strips in the matrix, following the dense traversal of the \acrshort{op} algorithm, as described in Section~\ref{sec:background} \note{FIX} and shown with \textit{red} arrows in Figure~\ref{fig:theory:model:matrix_cache_rep}.
