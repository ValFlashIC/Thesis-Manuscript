...
A second implementation constraint is that region sizes for \acrshort{grc}, \acrshort{drc}, and \acrshort{srt} must be powers of two in terms of \glspl{set}, unlike the other models.
This limitation motivates our choice of a 50\%/50\%/0\% configuration for \acrshort{redc}, which we expect to underperform relative to more flexible configurations.
Future optimizations of the implementation could relax this constraint.

\begin{highlight}{Conclusion}{Red}
    We evaluate three cache configurations, \acrshort{gc}, \acrshort{redc}, and \acrshort{spdc}, using a single-core CVA6 processor with a single cache level.
\end{highlight}
...
\begin{highlight}{Conclusion}{Red}
    We employ \acrshort{csc} format for most simulations and additionally evaluate \acrshort{spdc} using \acrshort{bacsc}, a specialized format optimized for \gls{banded} matrices.
\end{highlight}
...
Notably, when normalized per the number of non-zero elements ($nnz$), the \acrshort{gc} output memory traffic exhibits 17.6$\times$ higher traffic on group~1 than group~2, yet \acrshort{spdc} maintains significant relative benefits (see Section~\ref{sec:arch:perfGroups} for further discussion).
...
\begin{highlight}{Conclusion}{Red}
    ...
    Group~1 matrices (low $d_\mathit{cd}$) benefit most from traffic reduction due to sparse cache-lines, while group~2 (high $d_\mathit{cd}$) shows more modest gains as cache-line density mitigates the problem.
    Total memory traffic remains favorable for \acrshort{spdc}, with \acrshort{bacsc} overhead under 10\%, confirming the design's effectiveness across diverse matrix structures.
\end{highlight}
...
For group~1 matrices, \acrshort{redc} obtains modest speedup (1.1$\times$) because \acrshort{red} instructions are ``fire-and-forget'', replacing multiple read-accumulate-write operations.
However, blocking occurs when the \textit{accumulation table} processes prior \glspl{eviction}.
Group~1's sparse cache-lines trigger frequent \gls{eviction} bursts, dominating performance and making the system memory-bound.
...
In the extreme case, 7 \glspl{hit} following an \gls{eviction} require 56-98 instructions (8-14 per iteration, see Section~\ref{sec:arch:sw}) before the next potential \gls{eviction}, sufficient to hide the 64-cycle memory latency.

Consequently, \acrshort{spdc} with \acrshort{bacsc} matches \acrshort{gc} performance.
\acrshort{redc} underperforms by 1.1$\times$ due to its 2$\times$ smaller \acrshort{drc} versus \acrshort{gc}, larger \acrshort{drc} would yield comparable performance.
...
\begin{highlight}{Conclusion}{Red}
    ...
    For group~2 matrices (high $d_\mathit{cd}$), the workload becomes compute-bound and \acrshort{spdc}(\acrshort{bacsc}) still performs best by matching \acrshort{gc} performance.
    Overall, \acrshort{spdc}(\acrshort{bacsc}) achieves 1.1$\times$ average speedup, while the other configurations underperform on average, with \textbf{gains concentrated in memory-bound scenarios}.
\end{highlight}

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Further Discussion Over the Two Performance Groups}
\label{sec:arch:perfGroups}

In the two previous sections, we showed performance normalized to \acrshort{gc}, highlighting how \acrshort{spdc} affects baseline performance upon replacement.
...
For both metrics, white indicates better performance, though scales are not shared across graphs.
...
We observe that group~2 consistently outperforms group~1 across all configurations and metrics.
...
Repeated \gls{eviction} waiting causes processor stalls when $d_\mathit{cd}$ is low, inducing memory-bound behavior.
...
Once \gls{eviction} latency hides within processor execution, the workload becomes compute-bound.

Group~1 benefits significantly from memory traffic reduction, while group~2 is constrained by software implementation.
Previous sections demonstrated this against \acrshort{gc}.
While \acrshort{gc} and \acrshort{redc} remain bottlenecked, \acrshort{spdc} substantially reduces costly \glspl{eviction} and avoids bursting by accumulating with high \gls{hit}-rates in \acrshort{drc} or evicting via low-cost, element-wise transactions from \acrshort{srt}.
Consequently, \acrshort{spdc} delivers significant latency reduction in memory-bound group~1 alongside substantial traffic reduction.
For compute-bound group~2, traffic reduction yields modest latency improvements.
Here, \acrshort{spdc}(\acrshort{bacsc}) excels by reducing inner-loop instructions with negligible total traffic overhead.

For \acrshort{gc}, loop unrolling allows the \textit{miss handler}'s multiple entries to queue up to eight concurrent \gls{eviction} reads.
However, this proves insufficient for group~1 stalls.
Increasing the \textit{accumulation table} entries to eight similarly yields negligible speedup ($<1\%$) for \acrshort{redc} and \acrshort{spdc}.
...
\begin{highlight}{Conclusion}{Red}
    ...
    Consequently, \acrshort{spdc} achieves speedup in group~1 through memory traffic reduction, but only matches baseline performance in group~2 where software implementation becomes the bottleneck.
\end{highlight}
