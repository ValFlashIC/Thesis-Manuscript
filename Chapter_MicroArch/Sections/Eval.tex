...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Chapter_MicroArch/Figures/PDF/graph11_25.11.28.pdf}
    \caption{Normalized Output Memory Traffic Depending on the Out-of-Band Density for the \acrshort{redc} and \acrshort{spdc} Configurations}
    \label{fig:arch:graph11}
\end{figure}

% generate banded matrices with various d (IB 1e-2  OB -2 to -7) M=10000
% cache 32k (same configs as table 25/50/25  50/50)
% minor diff, follows the results -> ok!
% (add python??)

\subsection{Comparison with the Python Model}

To further validate our implementation, we compare the \acrshort{rtl} results against the Python simulations from Chapter~\ref{cha:spdc}.
...
\begin{highlight}{Conclusion}{Red}
    The \acrshort{rtl} implementation corroborates the Python simulations, exhibiting slightly improved memory traffic performance.
\end{highlight}
