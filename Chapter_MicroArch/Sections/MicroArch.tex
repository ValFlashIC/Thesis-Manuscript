...
% As the \textit{cache controller} is the heart of the cache process, the cache directory (\textit{Dir}) and data (\textit{Data}) RAMs its essence.
The \textit{cache controller} drives several components depending on the needs.
...
...
%% there is no transfer at eviction from DRC to RMWQ, give Python number to show it has low impact
...
\textbf{In the case of a \gls{hit}}, the value of the \acrshort{red} instruction is accumulated with its corresponding value in the cache.
...
\textbf{In the case of a \gls{miss}}, if there is an available cache-line, the value of the \acrshort{red} instruction is simply stored in this free entry of the cache.
...
\textbf{If a \gls{miss} occurs but no cache-line is free}, an \gls{eviction} must be processed before handling the \acrshort{red} instruction.
