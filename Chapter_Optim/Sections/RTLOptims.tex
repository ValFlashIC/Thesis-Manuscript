...
Supporting arbitrary region sizes would require integer division and modulo operations, which could introduce longer, critical paths.
...
\begin{itemize}
    ...
    %
    \item Region determination: For each $RED(addr, val)$ instruction, \acrlong{spdc} computes the current row $i \in \ldb 0; M-1 \rdb$ from the address: $i = \dfrac{addr-c}{s_w}$, then determines the region as \acrshort{ibr} if $|i-j| \leqslant w_\mathit{hB}$, otherwise \acrshort{obr}.
\end{itemize}
...
This feature becomes essential when extending \acrlong{spdc} to multi-level, multi-core architectures (Section~\ref{sec:optim:multi-core}).
