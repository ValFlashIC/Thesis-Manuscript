...
\begin{itemize}
    ...
    When this instruction is issued, prior to each \acrshort{spmv}, the half-\gls{band}-width is computed and retained until reconfigured, where $s_l$ denotes cache-line size in bytes:
    $$w_\mathit{hB} = \dfrac{s_\mathit{DRC}-s_l}{2s_w}+1$$
    The half-\gls{band}-width could also be computed in software, and given as argument to the configuration.
    %
...
A simple detection system could verify that \glspl{reduction} are issued exclusively on addresses within the output vector $c$'s range, while other instructions operate outside this range.
