...
The \acrshort{rtl} implementation of \acrlong{spdc} provided in Chapter~\ref{cha:arch} has been enough to get performance results in accordance with all the models, and allowed us to do a precise analysis of the cache performance depending on the matrix structure.
...
To support accumulations between \textit{st1} and \textit{st2} in the \gls{pipeline} (Section~\ref{sec:arch:pipeline}), we can either deploy a single-cycle high-performance FPU with area trade-offs, or extend the \gls{pipeline} to accommodate two-cycle, for example, floating-point accumulation.
