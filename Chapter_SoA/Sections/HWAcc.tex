...
In this section, we describe the recent (2016-2025) hardware accelerators for sparse \acrshort{mm} and \acrshort{mv}, targeting ASIC-type implementation.
We try to be consistent on notations and architecture schemes to simplify the comparison.
%%%% shorter?? --> FIXME
Before going into the detailed overviews of the accelerators, in Table~\ref{tab:soa:classificationOverview} we present a criteria-based classification of the designs.

A first criterion to classify the accelerators is between \emph{standalone} accelerators that address the full kernel without the assist of a processor (\eg OuterSPACE~\cite{Pal2018_OuterSPACE} -- Section~\ref{sec:soa:outerspace}) and ones that address only a part of the kernel and need a processor to manage the computation (\eg PHI~\cite{Mukkara2019_PHI} -- Section~\ref{sec:soa:other_accel}).%FIXME
Most of the \emph{standalone} accelerators are directly connected to the main memory and internally manage their own custom memories.
...
Certain accelerators intervene near the processors (\eg ASA~\cite{Zhang2022_ASA} -- Section~\ref{sec:soa:asa}), near the cache (\eg PHI and SortCache~\cite{Srikanth2021_SortCache} -- Sections~\ref{sec:soa:other_accel} and~\ref{sec:soa:sortcache}), or near the main memory (\eg SPiDRE~\cite{Barredo2019_SPiDRE} and SpaceA~\cite{Xie2021_SpaceA} -- Section~\ref{sec:soa:other_accel}).%FIXME
...
For example, some accelerators (\eg PHI and SortCache) address the \gls{reduction} problem and are adapted to \acrshort{op} algorithms but \glspl{reduction} are also required with \acrshort{gust} and highly-tiled algorithms.
ExTensor~\cite{Hegde2019_ExTensor} (Section~\ref{sec:soa:extensor}) is a general purpose accelerator that is demonstrated for a highly-tiled \acrshort{op} but also makes major contributions for \acrshort{ip}.%FIXME ADD
...
For example, some accelerators (\eg SpArch~\cite{Zhekai2020_SpArch} -- Section~\ref{sec:soa:sparch}) are specialized for an \acrshort{op} algorithm but optimize both \gls{gather} and \gls{gather}.
Finally, accelerators preferably focus on sparse \acrshort{mm}, \acrshort{mv} or both, our final criterion.

After this first comparison, in Sections~\ref{sec:soa:outerspace} to~\ref{sec:soa:gamma} we present the \emph{standalone} accelerators, followed by the \emph{processor assist} accelerators in Sections~\ref{sec:soa:sortcache} to~\ref{sec:soa:asa}.%FIXME
In Section~\ref{sec:soa:other_accel}, we briefly touch on other accelerators.

%note about SpDCache in the table
...
%             \hhline{==::==::==::==::==}
%             OuterSPACE & \acrshort{mm} & \emph{(UOP, CP) col.-maj.} & Main memory & \emph{(UOP, CP) row-maj.} & Main memory & \emph{(UOP, CP) col.-maj.} & Main memory & \emph{(UOP, CP) col.-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%             ExTensor & \acrshort{mm} & \emph{(UOP, CP)}, tiling & Main memory & \emph{(UOP, CP)}, tiling & Main memory & \emph{CP$^{2}$} & Buffer & \emph{CP$^{2}$} & Main memory \\
%             \hhline{--||--||--||--||--}
%             PHI & \acrshort{mv} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{CP} & Caches, Main memory \\
%             \hhline{--||--||--||--||--}
%             SPiDRE & \acrshort{mv} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \emph{CP $\xrightarrow{}$ RA} & Main memory & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{CP $\xrightarrow{}$ RA} & Main memory \\
%             \hhline{--||--||--||--||--}
%             MatRaptor & \acrshort{mm} & \emph{(FL, UOP, CP) channel-maj.} & Main memory & \emph{(FL, UOP, CP) channel-maj.} & Main memory & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{(UOP, CP) row-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%             SpArch & \acrshort{mm} & \emph{(UOP, CP) row-maj.}, condensed column & Main memory & \emph{(UOP, CP) row-maj.} & Main memory & \emph{CP$^{2}$ row-maj.} & Main memory & \emph{(UOP, CP) row-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%             Gamma & \acrshort{mm} & \emph{(UOP, CP) row-maj.} & Main memory & \emph{(UOP, CP) row-maj.} & Internal cache & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{(UOP, CP) row-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%             InnerSP & \acrshort{mm} & \emph{(UOP, CP) row-maj.} & Main memory & \emph{(UOP, CP) row-maj.} & Main memory & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{(UOP, CP) row-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%             SortCache & \acrshort{mm} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{(UOP, CP) $\xrightarrow{}$ VBST} & Caches \\
%             \hhline{--||--||--||--||--}
%             ASA & \acrshort{mm} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \multicolumn{2}{c||}{\cellcolor{veryLightGray} SW} & \emph{CP} & Internal cache, Main memory & \emph{(UOP, CP)} &SPiDRE Main memory \\
%             \hhline{--||--||--||--||--}
%             GSCSp & \acrshort{mm} & \emph{(UOP, CP) row-maj.} & Main memory & \emph{(UOP, CP) row-maj.} & Internal cache & \multicolumn{2}{c||}{\cellcolor{lightGray} NC} & \emph{(UOP, CP) row-maj.} & Main memory \\
%             \hhline{--||--||--||--||--}
%         \end{tabular}
