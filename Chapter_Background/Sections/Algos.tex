...
\begin{table}
    ...
    \footnotesize
    \begin{tabular}{|>{\centering\arraybackslash}m{1.49cm}|| *{3}{>{\arraybackslash}m{4.22cm}|}}
        \hhline{-||---}
        &
        \textbf{\acrfull{ip}}\hyperlink{fn1}{$^{\mathrm{1}}$} &
        \textbf{\acrfull{op}}\hyperlink{fn1}{$^{\mathrm{1}}$} &
        \textbf{\acrfull{gust}}\hyperlink{fn1}{$^{\mathrm{2}}$} \\
        %
        \hhline{=::===}
        \textbf{A$(M, K)$} \newline \textbf{Accesses} &
        $\bullet$ \emph{row-wise} \newline $\bullet$ read $N$ times\hyperlink{fn2}{$^{\mathrm{3}}$}\hyperlink{fn3}{$^{\mathrm{4}}$} \newline $\bullet$ sequential &
        ...
        % $\bullet$ generated \emph{row-wise} \newline $\bullet$ for each row of $A$ ($M$), $\overline{nnz_r}(A)$ \acrshort{po}-rows produced (each \acrshort{po}-rows has $\overline{nnz_r}(B)$ non-zero elements) \\
        $\bullet$ generated \emph{row-wise} \newline $\bullet$ for each row of $A$ ($M$), $K \times d(A)$ \acrshort{po}-rows produced\hyperlink{fn9}{$^{\mathrm{10}}$} (each \acrshort{po}-rows has $\overline{nnz_r}(B)$ non-zero elements) \\
        %
        \hhline{-||---}
        \multicolumn{4}{l}{\hypertarget{fn0}{$^{\mathrm{1}}$}Considering $N = 1$ in the case of \acrshort{mv} multiplication.} \\
        \multicolumn{4}{l}{\hypertarget{fn1}{$^{\mathrm{2}}$}Only for \acrshort{mm} multiplication.} \\
        \multicolumn{4}{l}{\hypertarget{fn2}{$^{\mathrm{3}}$}Or less than $N$ times if $B$ has empty columns.} \\
        \multicolumn{4}{l}{\hypertarget{fn3}{$^{\mathrm{4}}$}Read once in the case of \acrshort{mv} multiplication.} \\
        \multicolumn{4}{l}{\hypertarget{fn4}{$^{\mathrm{5}}$}If $B$ is stored with a sorted \gls{spFormat}, else it would be read $\overline{nnz_r}(A) \times M = nnz(A)$ times.} \\
        \multicolumn{4}{l}{\hypertarget{fn5}{$^{\mathrm{6}}$}\emph{element-wise} in the case of \acrshort{mv} multiplication (there is no row).} \\
        \multicolumn{4}{l}{\hypertarget{fn6}{$^{\mathrm{7}}$}Which is equal to $\overline{nnz_c}(A)$, the average number of non-zero elements in the columns of $A$.} \\
        \multicolumn{4}{l}{\hypertarget{fn7}{$^{\mathrm{8}}$}Or less than $K$ times if $A$ has empty columns or $B$ has empty rows.} \\
        \multicolumn{4}{l}{\hypertarget{fn8}{$^{\mathrm{9}}$}$\overline{nnz_c}(A)$ in the case of \acrshort{mv} multiplication.} \\
        \multicolumn{4}{l}{\hypertarget{fn9}{$^{\mathrm{10}}$}Which is equal to $\overline{nnz_r}(A)$, the average number of non-zero elements in the rows of $A$.}
    \end{tabular}
